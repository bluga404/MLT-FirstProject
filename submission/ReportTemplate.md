...
- **Solusi 2 (Hyperparameter Tuning)**: Melakukan optimasi pada model *baseline* terbaik (AdaBoost) menggunakan `GridSearchCV` dengan strategi validasi `TimeSeriesSplit` agar terhindar dari *temporal data leakage*. Tujuannya adalah untuk meningkatkan akurasi (mengurangi RMSE) dibandingkan model dengan parameter *default*.
...
Untuk model **AdaBoost** (model baseline dengan performa terbaik, yaitu RMSE terendah di antara tiga model ML baseline), dilakukan optimasi untuk mengurangi *error* lebih lanjut. Pencarian grid mendalam (*Grid Search CV*) dilakukan. Untuk menjaga keamanan deret waktu, diatur agar teknik validasi menggunakan iterasi maju `TimeSeriesSplit` dengan 3 lipatan.

**Parameter Grid yang Diuji:**

| Parameter | Nilai yang Diuji | Fungsi |
|---|---|---|
| `n_estimators` | [50, 100, 200, 300] | Jumlah iterasi boosting (jumlah model lemah) |
| `learning_rate` | [0.01, 0.1, 0.5, 1.0] | Kontribusi setiap model lemah terhadap prediksi akhir |
| `loss` | ['linear', 'square'] | Fungsi loss untuk update bobot |
...
|---|---|---|---|
| MSE | 50.465,32 | 53.802,31 | ↑ (memburuk) |
| RMSE | 224,64 | 231,95 | ↑ (memburuk) |
| MAE | 170,22 | 181,31 | ↑ (memburuk) |

**Interpretasi:** Tuning pada AdaBoost menggunakan `GridSearchCV` dengan `TimeSeriesSplit` tidak menghasilkan peningkatan performa. Parameter `learning_rate` turun dari 1.0 menjadi 0.1, yang berarti kontribusi setiap model lemah menjadi lebih kecil. Penurunan `learning_rate` seharusnya dikompensasi dengan peningkatan `n_estimators`, namun grid search justru memilih `n_estimators` tetap 50. Hal ini menunjukkan bahwa parameter default AdaBoost sudah cukup optimal untuk dataset ini, dan validasi silang deret waktu (`TimeSeriesSplit`) memberikan estimasi generalisasi yang lebih konservatif dibandingkan evaluasi pada data uji tunggal.
...
| Tuned AdaBoost | 53.802,31 | - | 231,95 | 181,31 |
...
**Tidak memberikan peningkatan.** Tuning pada AdaBoost (model baseline terbaik) menggunakan `GridSearchCV` dengan `TimeSeriesSplit` tidak berhasil menurunkan RMSE. Sebaliknya, RMSE justru meningkat dari 224,64 (default) menjadi 231,95 (tuned), yaitu peningkatan error sebesar ~3,3%.
- Parameter default AdaBoost (`n_estimators=50`, `learning_rate=1.0`) sudah cukup optimal untuk dataset ini.
- Validasi silang deret waktu (`TimeSeriesSplit`) memberikan estimasi generalisasi yang lebih konservatif, sehingga parameter yang dipilih grid search berbeda dari yang terbaik pada evaluasi data uji tunggal.
- **Implikasi bisnis:** Hyperparameter tuning tidak selalu menghasilkan peningkatan. Pemilihan model yang tepat (dalam hal ini Simple Linear Regression, RMSE 200,24) tetap memberikan dampak yang lebih besar daripada optimasi parameter.
